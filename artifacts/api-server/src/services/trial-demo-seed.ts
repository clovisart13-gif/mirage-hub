...
async function ensureBudgetDemoClient(tenantId: string, demoClient: (typeof DEMO_V2_CLIENTS)[number]) {
  const { data: existingClient, error: lookupError } = await supabaseAdmin
    .from("clientes")
    .select("id")
    .eq("tenant_id", tenantId)
    .eq("nome", demoClient.nome)
    .limit(1)
    .maybeSingle();
  if (lookupError) throw new Error(`Não foi possível consultar o cliente de orçamento ${demoClient.nome}: ${lookupError.message}`);
  if ((existingClient as any)?.id) return (existingClient as any).id as string;

  const { data, error } = await supabaseAdmin
    .from("clientes")
    .insert({
      tenant_id: tenantId,
      nome: demoClient.nome,
      email: demoClient.email,
      telefone: demoClient.telefone,
      ativo: true,
    } as any)
    .select("id")
    .single();
  if (error || !data) {
    const { data: retriedClient, error: retryError } = await supabaseAdmin
      .from("clientes")
      .select("id")
      .eq("tenant_id", tenantId)
      .eq("nome", demoClient.nome)
      .limit(1)
      .maybeSingle();
    if (retryError || !(retriedClient as any)?.id) {
      throw new Error(`Não foi possível criar o cliente de orçamento ${demoClient.nome}: ${error?.message ?? retryError?.message ?? "registro ausente"}`);
    }
    return (retriedClient as any).id as string;
  }
...
async function ensureDemoV2BudgetBase(tenantId: string, summary: SeedSummary) {
  ...
  for (const demo of demoBudgets) {
    const demoClient = DEMO_V2_CLIENTS.find((candidate) => candidate.nome === demo.cliente);
    if (!demoClient) throw new Error(`Cliente demonstrativo não configurado: ${demo.cliente}`);
    const clientId = await ensureBudgetDemoClient(tenantId, demoClient);
...
export async function hasTrialDemoV2Data(tenantId: string) {
  const [operationalResult, budgetResult] = await Promise.all([
    pool.query(
      "SELECT 1 FROM plm_produtos WHERE tenant_id = $1 AND referencia = $2 LIMIT 1",
      [tenantId, "TRIALV2-SHO-12"],
    ),
    supabaseAdmin
      .from("orcamentos")
      .select("id")
      .eq("tenant_id", tenantId)
      .eq("numero", "TRIALV2-ORC-003")
      .limit(1)
      .maybeSingle(),
  ]);
  if (budgetResult.error) throw new Error(`Não foi possível verificar a Base Demonstrativa v2: ${budgetResult.error.message}`);
  return operationalResult.rows.length > 0 && Boolean(budgetResult.data);
}
